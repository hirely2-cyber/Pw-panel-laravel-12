...
<?php $__env->startSection('header', 'Server Control'); ?>
<?php $__env->startSection('subheader', 'Game Server Management'); ?>
...
<div x-data="serverCtrl()" x-init="init()">
    ...
    <div class="pw-adm-card" style="padding:.8rem 1.2rem;margin-bottom:1rem;">
        ...
        <form method="POST" action="<?php echo e(route('admin.server-control.path')); ?>" style="display:flex;align-items:center;gap:.8rem;">
            <?php echo csrf_field(); ?>
            ...
            <input type="text" name="server_path" value="<?php echo e(rtrim($serverPath, '/')); ?>"
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__errorArgs = ['server_path'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                <span style="color:#e05252;font-size:.75rem;"><?php echo e($message); ?></span>
            <?php unset($message);
    ...
    <div class="pw-adm-card" style="padding:.9rem 1.2rem;margin-bottom:1rem;">
        <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:.6rem;">
            ...
            <div style="display:flex;flex-direction:column;gap:.18rem;">
                ...
                <span class="sc-val" style="font-size:.82rem;font-weight:700;color:var(--pw-text-light);"><?php echo e($serverInfo['hostname']); ?></span>
                <span style="font-size:.63rem;color:var(--pw-text-muted);"><?php echo e($serverInfo['os']); ?></span>
            </div>

            
            <div style="display:flex;flex-direction:column;gap:.18rem;">
                <span style="font-size:.58rem;font-weight:700;letter-spacing:.07em;color:var(--pw-text-muted);">CPU</span>
                <span class="sc-val" style="font-size:.82rem;font-weight:700;color:var(--pw-text-light);"><?php echo e($serverInfo['cpuCores']); ?> Core</span>
                <span style="font-size:.63rem;color:var(--pw-text-muted);" title="<?php echo e($serverInfo['cpuModel']); ?>"><?php echo e(Str::limit($serverInfo['cpuModel'], 28)); ?></span>
            </div>

            
            <div style="display:flex;flex-direction:column;gap:.18rem;">
                <span style="font-size:.58rem;font-weight:700;letter-spacing:.07em;color:var(--pw-text-muted);">LOAD AVG</span>
                <?php $l1 = (float)$serverInfo['load1']; $lc = $serverInfo['cpuCores']; ?>
                <span style="font-size:.82rem;font-weight:700;color:<?php echo e($l1 > $lc ? '#ef4444' : ($l1 > $lc*0.7 ? '#f59e0b' : '#50c878')); ?>;">
                    <?php echo e($serverInfo['load1']); ?>

                </span>
                <span style="font-size:.65rem;color:var(--pw-text-muted);">5m: <?php echo e($serverInfo['load5']); ?> &bull; 15m: <?php echo e($serverInfo['load15']); ?></span>
            </div>
            ...
            <div style="display:flex;flex-direction:column;gap:.18rem;">
                ...
                <div style="display:flex;align-items:baseline;gap:.3rem;">
                    <span class="sc-val" style="font-size:.82rem;font-weight:700;color:var(--pw-text-light);"><?php echo e($serverInfo['diskUsed']); ?></span>
                    <span style="font-size:.68rem;color:var(--pw-text-muted);">/ <?php echo e($serverInfo['diskTotal']); ?></span>
                </div>
                <div class="sc-bar-bg" style="height:4px;background:rgba(255,255,255,.08);border-radius:3px;margin-top:.15rem;">
                    <div style="height:100%;border-radius:3px;width:<?php echo e($serverInfo['diskPct']); ?>%;background:<?php echo e($serverInfo['diskPct'] > 90 ? '#ef4444' : ($serverInfo['diskPct'] > 75 ? '#f59e0b' : '#3b82f6')); ?>;"></div>
                </div>
                <span style="font-size:.63rem;color:var(--pw-text-muted);"><?php echo e($serverInfo['diskPct']); ?>% used</span>
            </div>

            
            <div style="display:flex;flex-direction:column;gap:.18rem;">
                <span style="font-size:.58rem;font-weight:700;letter-spacing:.07em;color:var(--pw-text-muted);">UPTIME</span>
                <span class="sc-val" style="font-size:.82rem;font-weight:700;color:var(--pw-text-light);"><?php echo e(Str::after($serverInfo['uptime'], 'up ')); ?></span>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem;">
        ...
        <div class="pw-adm-card" style="padding:1.2rem;display:flex;flex-direction:column;gap:.9rem;">
            ...
            <div class="sc-info-box" style="font-size:.68rem;color:var(--pw-text-muted);background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:5px;padding:.4rem .7rem;display:flex;align-items:center;gap:.4rem;">
                ...
                <span>Backup DB disimpan ke: <code style="font-family:monospace;color:#c4b5fd;"><?php echo e(rtrim($serverPath, '/')); ?>/pw_backup_YYYYMMDD.tar.gz</code></span>
            </div>
...
<div class="pw-adm-card" style="padding:1rem 1.2rem;margin-top:1rem;">
    ...
    <div style="overflow:auto;">
        <table style="width:100%;border-collapse:collapse;min-width:980px;">
            ...
            <tbody>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $actionLogs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                    <tr>
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.78rem;white-space:nowrap;"><?php echo e($row->created_at?->format('d/m/Y H:i:s')); ?></td>
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.78rem;"><?php echo e($row->actor_name ?: '-'); ?></td>
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.78rem;text-transform:uppercase;"><?php echo e($row->actor_role ?: '-'); ?></td>
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.78rem;text-transform:uppercase;"><?php echo e($row->panel_area ?: '-'); ?></td>
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.78rem;font-family:monospace;"><?php echo e(strtoupper((string) $row->action)); ?></td>
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.78rem;font-family:monospace;"><?php echo e($row->target_map ?: '-'); ?></td>
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.78rem;text-align:right;"><?php echo e((int) $row->delay_seconds); ?></td>
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.78rem;">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($row->result_ok): ?>
                        ...
                        <td style="padding:.55rem;border-bottom:1px solid var(--pw-border);font-size:.74rem;color:var(--pw-text-muted);max-width:340px;word-break:break-word;"><?php echo e($row->result_message ?: '-'); ?></td>
                    </tr>
...
function serverCtrl() {
    return {
        pathConfigured: <?php echo e($pathConfigured ? 'true' : 'false'); ?>,
        serverRunning:     <?php echo e($serverRunning ? 'true' : 'false'); ?>,
        anyDaemonRunning:  <?php echo e($anyDaemonRunning ? 'true' : 'false'); ?>,
        backupRunning:  <?php echo e(($processInfo['backupRunning'] ?? false) ? 'true' : 'false'); ?>,
        mapsCount:      <?php echo e(count($processInfo['maps'] ?? [])); ?>,
        maps:           <?php echo json_encode((object)($processInfo['maps'] ?? [])); ?>,
        memory:         <?php echo json_encode($memory); ?>,
        allMaps:        <?php echo json_encode($availableMaps); ?>,
        daemons: <?php echo json_encode(collect($processInfo['daemons'] ?? [])->map(function($d) {
        ...
        async doAction(action, mapId) {
            ...
            this.busyMsg = action === 'stop' ? 'Menghentikan server...'
                         ...
                         : 'Memproses...';
            try {
                const body = new URLSearchParams({ action, _token: '<?php echo e(csrf_token()); ?>' });
                if (mapId) body.append('map', mapId);
                if (action === 'stopall') body.append('delay', this.stopAllDelay);

                const res  = await fetch('<?php echo e(route("admin.server-control.action")); ?>', { method: 'POST', body });
        ...
        async refreshStatus() {
            try {
                const data = await fetch('<?php echo e(route("admin.server-control.status")); ?>').then(r => r.json());
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/pw-panel/resources/views/admin/server-control.blade.php ENDPATH**/ ?>