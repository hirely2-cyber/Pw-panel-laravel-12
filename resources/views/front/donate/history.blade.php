...
@section('title', 'Riwayat Transaksi — ' . $__siteName)
...
<section class="pw-section">
    <div class="pw-section__inner pw-section__inner--narrow">
        ...
        <div class="pw-shist-topbar">
            ...
            @if(!$invoices->isEmpty())
            <span class="pw-shist-topbar__total">{{ $invoices->total() }} transaksi</span>
            @endif
        </div>
        ...
        <div class="pw-shop-empty">
            ...
            <a href="{{ route('donate') }}" class="pw-btn pw-btn--gold pw-btn--sm" style="margin-top:.4rem;">Top-up Gold Points Sekarang</a>
        </div>
        @else
        ...
        <table style="width:100%;border-collapse:collapse;font-size:.83rem;">
            ...
            <tr style="border-bottom:1px solid rgba(255,255,255,.05);transition:background .12s;" onmouseover="this.style.background='rgba(255,255,255,.03)'" onmouseout="this.style.background=''">
                <td style="padding:.6rem .8rem;color:var(--pw-text-muted);">{{ $invoices->firstItem() + $i }}</td>
                <td style="padding:.6rem .8rem;font-family:monospace;font-size:.78rem;color:var(--pw-text-muted);">{{ $inv->invoice_number }}</td>
                <td style="padding:.6rem .8rem;text-align:right;font-weight:700;color:var(--pw-gold);">+{{ number_format($inv->gold_amount) }}</td>
                <td style="padding:.6rem .8rem;text-align:right;font-weight:600;color:var(--pw-text);">Rp {{ number_format($inv->unique_amount) }}</td>
                <td style="padding:.6rem .8rem;text-align:center;">
                    <span class="pw-shist-badge {{ $s['class'] }}">{{ $s['label'] }}</span>
                </td>
                <td style="padding:.6rem .8rem;color:var(--pw-text-muted);white-space:nowrap;font-size:.78rem;">{{ $inv->created_at->format('d M Y, H:i') }}</td>
                <td style="padding:.6rem .8rem;text-align:right;">
                    @if($inv->status === 'pending')
                    <a href="{{ route('donate.invoice.show', $inv->invoice_number) }}"
