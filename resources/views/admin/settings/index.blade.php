...
<form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="pw-adm-card" style="margin-bottom:1.25rem;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:.7rem;">
            ...
            <label class="pw-feat-toggle {{ $isOn ? 'is-on' : '' }}" style="display:flex;align-items:center;justify-content:space-between;gap:.8rem;padding:.75rem 1rem;background:rgba(255,255,255,.03);border:1px solid {{ $isOn ? 'rgba(200,151,42,.3)' : 'rgba(255,255,255,.07)' }};border-radius:8px;cursor:pointer;transition:border-color .15s;">
                <div>
                ...
                <div style="position:relative;flex-shrink:0;width:42px;height:24px;">
                    ...
                    <input type="checkbox" name="feature_{{ $key }}" value="1"
                        ...
                        {{ $isOn ? 'checked' : '' }}>
                    <div class="pw-feat-track" style="position:absolute;inset:0;border-radius:12px;background:{{ $isOn ? 'var(--pw-gold)' : 'rgba(255,255,255,.12)' }};transition:background .2s;pointer-events:none;"></div>
