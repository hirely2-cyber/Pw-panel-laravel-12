@extends('layouts.admin')
@section('title', $site->exists ? 'Edit Site Vote' : 'Tambah Site Vote')

@section('content')
<div>
    <div style="margin-bottom:1rem;">
        <a href="{{ route('admin.vote.index') }}" class="pw-adm-btn pw-adm-btn--ghost pw-adm-btn--sm">← Kembali</a>
    </div>

    <div class="pw-adm-card">
        <div class="pw-adm-card__title">{{ $site->exists ? 'Edit' : 'Tambah' }} Site Vote</div>

        <form action="{{ $site->exists ? route('admin.vote.update', $site->id) : route('admin.vote.store') }}"
              method="POST">
            @csrf
            @if($site->exists) @method('PUT') @endif

            <label class="pw-form__label">Nama Site <span style="color:#e05252;">*</span></label>
            <input type="text" name="name" class="pw-form__input" required
                   value="{{ old('name', $site->name) }}" placeholder="Top100Arena, MPOG, dll."
                   style="margin-bottom:.8rem;">
            @error('name') <p style="color:#e05252;font-size:.75rem;margin-top:-.6rem;margin-bottom:.6rem;">{{ $message }}</p> @enderror

            <label class="pw-form__label">URL Vote <span style="color:#e05252;">*</span></label>
            <input type="url" name="url" class="pw-form__input" required
                   value="{{ old('url', $site->url) }}" placeholder="https://top100arena.com/vote/..."
                   style="margin-bottom:.8rem;">
            @error('url') <p style="color:#e05252;font-size:.75rem;margin-top:-.6rem;margin-bottom:.6rem;">{{ $message }}</p> @enderror

            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.8rem;margin-bottom:.8rem;">
                <div>
                    <label class="pw-form__label">Reward Gold <span style="color:#e05252;">*</span></label>
                    <input type="number" name="reward" class="pw-form__input" required min="1"
                           value="{{ old('reward', $site->reward) }}">
                </div>
                <div>
                    <label class="pw-form__label">Urutan Tampil</label>
                    <input type="number" name="sort_order" class="pw-form__input" min="0"
                           value="{{ old('sort_order', $site->sort_order ?? 0) }}">
                </div>
            </div>

            <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;font-size:.85rem;margin-bottom:1.2rem;">
                <input type="hidden" name="is_active" value="0">
                <input type="checkbox" name="is_active" value="1"
                       {{ old('is_active', $site->is_active ?? true) ? 'checked' : '' }}>
                Aktifkan site vote
            </label>

            <div style="display:flex;gap:.5rem;">
                <button type="submit" class="pw-adm-btn" style="flex:1;">
                    {{ $site->exists ? 'Simpan' : 'Tambahkan' }}
                </button>
                <a href="{{ route('admin.vote.index') }}" class="pw-adm-btn pw-adm-btn--ghost">Batal</a>
            </div>
        </form>
    </div>
</div>
@endsection
