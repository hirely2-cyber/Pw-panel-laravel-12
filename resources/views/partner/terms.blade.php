...
@section('title', $lang === 'en' ? 'Terms & Conditions' : 'Peraturan & Ketentuan')
...
<div class="pw-adm-card">
    <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem;margin-bottom:1.25rem;padding-bottom:.75rem;border-bottom:1px solid var(--pw-border);">
        <div class="pw-adm-card__title" style="font-size:1rem;font-weight:700;margin:0;">
            ...
            {{ $lang === 'en' ? 'Terms & Conditions — Partner Program' : 'Peraturan & Ketentuan Program Partner' }}
        </div>
        <div style="display:flex;gap:.4rem;">
            <a href="{{ route('partner.terms') }}?lang=id"
               style="display:flex;align-items:center;gap:.3rem;padding:.35rem .75rem;border-radius:6px;font-size:.78rem;font-weight:600;text-decoration:none;border:1px solid;
                      {{ $lang === 'id' ? 'background:rgba(200,151,42,.15);border-color:rgba(200,151,42,.4);color:var(--pw-gold);' : 'background:rgba(255,255,255,.04);border-color:rgba(255,255,255,.1);color:var(--pw-text-muted);' }}">
                🇮🇩 Indonesia
            </a>
            <a href="{{ route('partner.terms') }}?lang=en"
               style="display:flex;align-items:center;gap:.3rem;padding:.35rem .75rem;border-radius:6px;font-size:.78rem;font-weight:600;text-decoration:none;border:1px solid;
                      {{ $lang === 'en' ? 'background:rgba(200,151,42,.15);border-color:rgba(200,151,42,.4);color:var(--pw-gold);' : 'background:rgba(255,255,255,.04);border-color:rgba(255,255,255,.1);color:var(--pw-text-muted);' }}">
                🇬🇧 English
            </a>
        </div>
    </div>
    ...
    <div style="margin-top:1.5rem;padding-top:.75rem;border-top:1px solid var(--pw-border);font-size:.72rem;color:var(--pw-text-muted);">
        {{ $lang === 'en' ? 'Last updated:' : 'Terakhir diperbarui:' }} {{ $terms->updated_at->format('d M Y H:i') }}
    </div>
    @endif
    @else
    <div style="text-align:center;padding:2.5rem 1rem;color:var(--pw-text-muted);font-size:.85rem;">
        {{ $lang === 'en' ? 'Terms & Conditions have not been set yet.' : 'Syarat & Ketentuan belum diisi oleh Administrator.' }}
    </div>
    @endif
</div>
