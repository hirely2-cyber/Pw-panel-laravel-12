...
@section('title', $article->title . ' — ' . $__siteName)
@section('meta_description', Str::limit(strip_tags($article->content), 160))

@section('content')

{{-- ============================================================
     ARTICLE HERO — Thumbnail bg + Judul + Meta + Breadcrumb
============================================================ --}}
<div class="pw-article-hero"
    @if($article->thumbnail)
        style="--article-thumb: url('{{ Storage::url($article->thumbnail) }}')"
    @endif>
    ...
    <div class="pw-article-hero__inner">
        ...
        <nav class="pw-breadcrumb" aria-label="breadcrumb">
            ...
            <a href="{{ route('news.index') }}" class="pw-breadcrumb__item">{{ __('main.news_page_title') }}</a>
            <span class="pw-breadcrumb__sep" aria-hidden="true">/</span>
            <span class="pw-breadcrumb__item pw-breadcrumb__item--active">{{ Str::limit($article->title, 45) }}</span>
        </nav>

        {{-- Category badge --}}
        @if($article->category)
        <span class="pw-article-hero__cat">{{ $article->category }}</span>
        @endif

        {{-- Title --}}
        <h1 class="pw-article-hero__title">{{ $article->title }}</h1>
...
<section class="pw-section pw-section--article">
    <div class="pw-article-wrap">
        ...
        <div class="pw-article__actions">
            <a href="{{ route('news.index') }}" class="pw-btn pw-btn--ghost">
...
<section class="pw-section pw-section--related">
    <div class="pw-section__inner pw-section__inner--narrow">
        <div class="pw-section__head">
            ...
            <h2 class="pw-section__title">{{ __('main.news_other') }}</h2>
            <div class="pw-section__ornament-line"></div>
        </div>
        <div class="pw-news-grid pw-news-grid--full">
            @foreach($recent as $item)
            <a href="{{ route('news.show', $item->slug) }}" class="pw-news-card">
                <div class="pw-news-card__thumb">
                    @if($item->thumbnail)
                        <img src="{{ Storage::url($item->thumbnail) }}" alt="{{ $item->title }}" loading="lazy">
                    @else
                        <svg viewBox="0 0 400 240" xmlns="http://www.w3.org/2000/svg">
                            <defs><linearGradient id="rng{{ $loop->index }}" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="{{ ['#0d1f3c','#1a2010','#2a0d0d','#1a0d2e'][$loop->index % 4] }}"/><stop offset="100%" stop-color="#0a0a14"/></linearGradient></defs>
                            <rect width="400" height="240" fill="url(#rng{{ $loop->index }})"/>
                            <circle cx="200" cy="120" r="22" fill="#c8972a" opacity=".2"/>
                        </svg>
                    @endif
                    <div class="pw-news-card__overlay"></div>
                    @if($item->category)
                    <span class="pw-news-card__cat">{{ $item->category }}</span>
                    @endif
                </div>
                <div class="pw-news-card__body">
                    <h3 class="pw-news-card__title">{{ Str::limit($item->title, 65) }}</h3>
