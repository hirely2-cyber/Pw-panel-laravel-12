...
@section('title', __('main.download_title') . ' — ' . $__siteName)
@section('meta_description', __('main.download_title') . ' ' . $__siteName)
...
<div class="pw-page-hero">
    ...
    <div class="pw-page-hero__inner">
        ...
        <h1 class="pw-page-hero__title">{{ __('main.download_title') }}</h1>
        <p class="pw-page-hero__sub">{{ __('main.download_subtitle') }}</p>
...
<section class="pw-section" id="download">
    <div class="pw-section__inner">
        ...
        <div class="pw-download-grid">
            ...
            <div class="pw-card pw-download-card" style="text-align:center;display:flex;flex-direction:column;align-items:center;position:relative;{{ $dl['key'] === 'full' ? 'border-color:rgba(232,184,75,.25);' : '' }}">

                @if($dl['badge'])
                <div style="position:absolute;top:-.6rem;left:50%;transform:translateX(-50%);background:{{ $dl['color'] }};color:#0a0a14;font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:.08em;padding:.2rem .75rem;border-radius:20px;">
                    {{ $dl['badge'] }}
                </div>
                @endif

                {{-- Icon --}}
                <div class="pw-download-icon-circle" style="width:56px;height:56px;border-radius:50%;background:rgba(255,255,255,.04);border:1px solid {{ $dl['color'] }}33;display:flex;align-items:center;justify-content:center;margin-bottom:1.25rem;">
                    <svg viewBox="0 0 20 20" fill="none" width="24" height="24" style="color:{{ $dl['color'] }}">
                        {!! $dl['icon'] !!}
                    </svg>
                </div>
                ...
                @if($dl['url'])
                <a href="{{ $dl['url'] }}" class="pw-btn {{ $dl['key'] === 'full' ? 'pw-btn--gold pw-btn--glow' : 'pw-btn--outline' }}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:.4rem;width:100%;justify-content:center;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="pw-sysreq__label">
                                <svg viewBox="0 0 20 20" fill="none" width="16" height="16" class="pw-sysreq__icon">{!! $spec['icon'] !!}</svg>
                                <span>{{ $spec['label'] }}</span>
                            </td>
                            <td class="pw-sysreq__val pw-sysreq__val--min">{{ $spec['min'] }}</td>
                            <td class="pw-sysreq__val pw-sysreq__val--rec">{{ $spec['rec'] }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
